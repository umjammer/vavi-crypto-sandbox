...
public class AES {
	...
	public static class rijndael_ctx {
		...
		public final int[] ek = new int[4*(AES_MAXROUNDS + 1)];
        /** decrypt key schedule */
		public final int[] dk = new int[4*(AES_MAXROUNDS + 1)];
	...
	private static void doCipher(rijndael_ctx ctx, int mode, byte[] src, int srcOffset, byte[] dst, int dstOffset, int size) {
		...
		try {
			...
			cipher.doFinal(src, srcOffset, alignUp(size, 15), dst, dstOffset);
                 ...
                 BadPaddingException | ShortBufferException | IllegalArgumentException e) {
			log.log(Level.SEVERE, "doCipher", e);
		}
    }
...
//#region AES-CMAC Generation Function
