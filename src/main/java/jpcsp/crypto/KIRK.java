...
public class KIRK {
    ...
    // KIRK header structs.
    private static class SHA1_Header {
...
//#region KIRK commands: main emulated crypto functions.
...
//        }
        ...
        int paddedElfDataSize = elfDataSize + 15 & ~15;
...
//        }
        ...
        size = Math.min(size, header.dataSize);
...
//        }
    ...
    private int executeKIRKCmd15(ByteBuffer out, int outSize, ByteBuffer in, int inSize) {
    	...
    	long output = input + 1;
        out.putLong(Long.reverseBytes(output));
...
//#region HLE functions

    // high level implementation of crypto functions from
    ...
    public int hleUtilsBufferCopyWithRange(ByteBuffer out, int outsize, ByteBuffer in, int insizeAligned, int insize, int cmd) {
    	if (useLibkirk) {
            ...
            return switch (cmd) {
                case PSP_KIRK_CMD_DECRYPT_PRIVATE -> executeKIRKCmd1(out, in, insizeAligned);
                case PSP_KIRK_CMD_ENCRYPT -> executeKIRKCmd4(out, in, insizeAligned);
                case PSP_KIRK_CMD_ENCRYPT_FUSE -> executeKIRKCmd5(out, in, insizeAligned);
                case PSP_KIRK_CMD_DECRYPT -> executeKIRKCmd7(out, in, insizeAligned);
                case PSP_KIRK_CMD_DECRYPT_FUSE -> executeKIRKCmd8(out, in, insizeAligned);
                case PSP_KIRK_CMD_PRIV_SIG_CHECK -> executeKIRKCmd10(in, insizeAligned);
                case PSP_KIRK_CMD_SHA1_HASH -> executeKIRKCmd11(out, in, insizeAligned);
                case PSP_KIRK_CMD_ECDSA_GEN_KEYS -> executeKIRKCmd12(out, outsize);
                case PSP_KIRK_CMD_ECDSA_MULTIPLY_POINT -> executeKIRKCmd13(out, outsize, in, insize);
                case PSP_KIRK_CMD_PRNG -> executeKIRKCmd14(out, insizeAligned);
                case PSP_KIRK_CMD_ECDSA_SIGN -> executeKIRKCmd16(out, outsize, in, insize);
                case PSP_KIRK_CMD_ECDSA_VERIFY -> executeKIRKCmd17(in, insize);
    ...
    private int libkirkUtilsBufferCopyWithRange(ByteBuffer out, int outsize, ByteBuffer in, int insizeAligned, int insize, int cmd) {
    	...
    	switch (cmd) {
    		...
    		case PSP_KIRK_CMD_DECRYPT_FUSE:
    			dataSize = ByteUtil.readBeInt(inbuff, 16);
                outsize = dataSize + 15 & ~15;
        		break;
			case PSP_KIRK_CMD_ENCRYPT:
			case PSP_KIRK_CMD_ENCRYPT_FUSE:
        		outsize = ByteUtil.readBeInt(inbuff, 16) + 20;
        		break;
			case PSP_KIRK_CMD_DECRYPT_PRIVATE:
				dataSize = ByteUtil.readBeInt(inbuff, 112);
                outsize = dataSize + 15 & ~15;
				break;
    	}
